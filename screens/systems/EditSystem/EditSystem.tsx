...
import { useUpdateSystem } from '../../../api/systems/useUpdateSystem';
...
const EditSystem: React.FC<EditSystemProps> = ({ systemId }) => {
  ...
  useEffect(() => {
    setBasedOnValue(system?.basedOn ?? '');
  }, [setBasedOnValue, system?.basedOn]);

  useEffect(() => {
    setShortDescriptionValue(system?.shortDescription ?? '');
  }, [setShortDescriptionValue, system?.shortDescription]);
  ...
  }, [nameValue, basedOnValue, shortDescriptionValue, updateSystemMutation, systemId]);
          ...
          <Col css={{ flexGrow: 5, flexBasis: '10rem' }}>
            <ContentSection flexWrap="wrap" direction="column">
              <Row fullWidth gap="md" justifyContent="between">
                <Col css={{ width: '400px' }}>
                  <TitleH2 marginBottom="md">Edit system</TitleH2>
                  <Input
                    disabled={disabled}
                    id="system-name"
                    label="System name"
                ...
                <Col alignSelf="stretch" css={{ flexGrow: 1, flexBasis: '25rem' }}>
                  <Col gap="md" alignItems="center">
                    ...
                    <ErrorText error={updateSystemMutation.error} />
                  </Col>
                </Col>{' '}
              </Row>
            </ContentSection>
            {moduleId > 0 && (
              <>
                <ContentSection flexWrap="wrap" direction="column" header="Tags">
                  <Text>
                    Select tags that describe your system. This will make it easier for other users
                    to find it.
                  </Text>
